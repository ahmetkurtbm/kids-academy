...
export async function GET() {
  ...
  try {
    ...
    const [{ data: parents, error: pError }, { data: exams, error: eError }, { data: events, error: eventError }, { data: announcements, error: announcementError }] = await Promise.all([
      ...
      db.from("events").select("id,title,event_date,summary,details,image_path,image_url,published,share_scope,parent_id,created_at").order("event_date", { ascending: false }).order("created_at", { ascending: false }),
      db.from("announcements").select("id,title,body,image_path,image_url,published,created_at").order("created_at", { ascending: false }),
    ]);
    if (pError || eError) throw pError || eError;
    return NextResponse.json({ parents, exams, events: eventError ? [] : events, announcements: announcementError ? [] : announcements, contentSetupRequired: Boolean(eventError || announcementError) });
  } catch (error) {
