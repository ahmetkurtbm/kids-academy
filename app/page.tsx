...
async function getEvents(): Promise<PublicEvent[]> {
  try {
    const { data, error } = await getSupabaseAdmin().from("events").select("id,title,event_date,summary,details,image_url").eq("published", true).order("event_date", { ascending: false }).order("created_at", { ascending: false }).limit(6);
    if (error) return [];
    return data || [];
  } catch { return []; }
}
...
const programs = [
  ...
  ["03", "★", "Jimnastik", "Denge, esneklik, koordinasyon ve hareket gelişimi", "yellow"],
...
const weekly = [
  ...
  ["BELİRLENEN GÜNLER", "Jimnastik", "Denge, koordinasyon ve temel hareket çalışmaları"],
...
export default async function Home() {
  ...
  return <main className="homeV2">
    ...
    <nav className="nav shell"><a className="logo" href="#"><span>KA</span><b>Kids Academy<small>eğitim & aktivite merkezi</small></b></a><div className="links"><a href="#programlar">Programlar</a><a href="#etkinlikler">Etkinlikler</a><a href="#haftalik-program">Haftalık Program</a><a href="#basarilar">Başarılar</a></div><a className="navCta" href="#iletisim">İletişime geç <Arrow /></a></nav>

    <section className="hero shell"><div className="heroCopy"><div className="eyebrow"><i>●</i> Osmaniye&apos;de akademik destek ve aktivite bir arada</div><h1>Öğrenirken güçlen,<br/><em>keşfederken büyü.</em></h1><p>Ödev takibi, konu anlatımı, jimnastik, yaz okulu ve zengin atölyelerle çocukların hem akademik hem sosyal gelişimini destekliyoruz.</p><div className="heroActions"><a className="primary" href="#programlar">Programları incele <Arrow /></a><a className="secondary" href="#iletisim">Bize ulaşın</a></div><div className="serviceTags"><span>✓ Planlı çalışma</span><span>✓ Bireysel takip</span><span>✓ Aktif öğrenme</span></div></div><div className="heroVisual"><div className="heroImage academyHero" role="img" aria-label="Kids Academy ödev ve etkinlik alanı"/><div className="floatCard available"><span>✎</span><div><small>Akademik destek</small><b>Ödev & konu anlatımı</b></div></div><div className="floatCard note"><span>★</span><div><small>Aktif gelişim</small><b>Jimnastik & atölye</b></div></div></div></section>
