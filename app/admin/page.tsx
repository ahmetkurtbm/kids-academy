...
export default function AdminPage() {
  ...
  const load = useCallback(async () => {
    ...
    setParents(data.parents || []); setExams(data.exams || []); setLoggedIn(true); setReady(true);
  }, []);
  useEffect(() => { load(); }, [load]);
  ...
  return <main className="adminShell">
    <aside className="adminSidebar"><a className="portalLogo inversePortal" href="/">KA <b>Kids Academy</b></a><nav><a href="#overview">Genel Bakış</a><a href="#new-exam">Deneme Girişi</a><a href="#parents">Veli Hesapları</a></nav><button onClick={async()=>{await fetch("/api/admin/logout",{method:"POST"});location.reload();}}>Çıkış yap</button></aside>
    <div className="adminMain"><header><div><small>YÖNETİM PANELİ</small><h1>Merhaba 👋</h1></div><a href="/" target="_blank">Siteyi görüntüle ↗</a></header>{message && <div className="formMessage">{message}</div>}
      <section id="overview" className="stats"><article><small>VELİ HESABI</small><b>{parents.length}</b></article><article><small>TOPLAM DENEME</small><b>{exams.length}</b></article><article><small>SON KAYIT</small><b className="smallStat">{exams[0]?.exam_date ? new Date(exams[0].exam_date).toLocaleDateString("tr-TR") : "—"}</b></article></section>
